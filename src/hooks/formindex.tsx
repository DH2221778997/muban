...
import { WechatMobileForm } from './WechatMobileForm';

// import moment from 'antd/node_modules/moment';
import moment from 'moment';
import {
  getMessageItem,
  registerNodeExtraToBackEnd,
  registerNodeOutputToBackEnd,
} from './utils/workfolwData';
import { WechatMessageForm } from './WechatMessageForm';
...
export const NodeEditForm: FC = () => {
  ...
  const getDefaultFormValue = (value) => {
    ...
    return {
      ...
      workflowParams: workflowParams ? { ...workflowParams } : undefined,
      aiCallOutput,
      setParamsOutput,
      smsTemplate,
      syncType,
  ...
  const getFormValueFromWechatMessage = (value) => {
    return {
      ...value.frontParams,
      ...value,
    };
  ...
  const getFormValueForWechat = (value) => {
    if (!value.extra) {
      value.extra = {
        hello: '',
        wxIdList: [],
  ...
  const filterFormValueByNodeType = (type) => {
    switch (type) {
      ...
      case WorkflowNodeType.WECHAT:
        return getFormValueForWechat;
      case WorkflowNodeType.WECHAT_MESSAGE:
        return getFormValueFromWechatMessage;
      default:
        /** 处理以前的逻辑 */
  ...
  const renderForm = () => {
    ...
    const usableOutput = findParentNodeIds(formValue!.id, dataSource.lines)
      .map((itemId) =>
        dataSource.nodes.find(({ id, nodeType, output }) => {
          return (
            id === itemId &&
            ([WorkflowNodeType.START, WorkflowNodeType.AI_CALL, WorkflowNodeType.REGISTER].includes(
              nodeType,
            ) ||
              !!output)
          );
    ...
    console.log('usableOutput', usableOutput);
    console.log('-'.repeat(20));
    const options = usableOutput.map(({ label, startOutput, aiCallOutput, output, nodeType }) => ({
      value: label,
      label,
      nodeType,
      children: (startOutput || aiCallOutput || output || []).map((item) => {
        console.log(item, 'get from options');
        return {
    ...
    switch (formValue!.nodeType) {
      ...
      case WorkflowNodeType.WECHAT_MESSAGE: {
        return <WechatMessageForm {...commonProps} />;
      }
      case WorkflowNodeType.WECHAT: {
        return <WechatForm {...commonProps} />;
      }
      case WorkflowNodeType.WECHAT_MOBILE: {
        return <WechatMobileForm {...commonProps} />;
      }
      case WorkflowNodeType.WECHAT_LABEL: {
        return <WechatMobileForm {...commonProps} />;
      }
      // 暂时不支持编辑
  ...
  const filterValueForWechatMessage = (values) => {
    return dataSource!.nodes.map((item) => {
      if (item.id === formValue?.id) {
        // 转换后端需要的 extra.replaceParams
        item.extra = {
          msgBOS: values?.msgBOS?.map?.(getMessageItem),
  ...
  const filterValueForWechat = (values: any) => {
    return dataSource!.nodes.map((item) => {
      if (item.id === formValue?.id) {
        item.label = values.label;
        item.extra.wxIdList = values.extra?.wxIdList;
        item.extra.hello = values.extra?.hello;
        item.extra.syncType = values.extra?.syncType ? 1 : 0;
        item.branches = [values.extra?.hello];
        return {
          ...values,
          ...item,
        };
  ...
  const filterValueForWechatModel = (values: any) => {
    return dataSource!.nodes.map((item) => {
      if (item.id === formValue?.id) {
        item.label = values.label;
        return {
  ...
  const filterValueByFormType = () => {
    switch (formValue?.nodeType) {
      ...
      case WorkflowNodeType.WECHAT:
        return filterValueForWechat;
      case WorkflowNodeType.WECHAT_MESSAGE:
        return filterValueForWechatMessage;
      case WorkflowNodeType.WECHAT_MOBILE:
        return filterValueForWechatModel;
      case WorkflowNodeType.WECHAT_LABEL:
        return filterValueForWechatModel;
      default:
